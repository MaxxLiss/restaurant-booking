...
class BookingServiceTest {
    ...
    @BeforeEach
    fun setup() {
        service = BookingService(TestUnitOfWork(restaurantRepo, userRepo, bookingRepo))
    }
    ...
    @Test
    fun `processPayment accrues 5 percent loyalty bonus`() {
        val (user, restaurant, table) = buildWorld()
        val dish = Dish("Стейк", BigDecimal("1000.00"), "Main")
        restaurant.menu.addDish(dish)
        val booking = Booking(user, table, startAt, endAt, 2)
        ...
        assertEquals(BigDecimal("50.00"), result.bonusAccrued)
        assertEquals(BigDecimal("50.00"), user.loyaltyAccount?.balance)
    }
}
